...
to align  ; wolf procedure
  let nearby-wolves other wolves in-radius 5  ; adjust the radius as needed
  ifelse any? nearby-wolves
    [ let avg-heading mean [heading] of nearby-wolves
      set heading towards one-of nearby-wolves ]
    [ rt random 50 lt random 50 ]  ; if no nearby wolves, move randomly
end


to cohese  ; wolf procedure
  let nearby-wolves wolves in-radius 5  ; adjust the radius as needed
  ifelse any? nearby-wolves
    [ let avg-x mean [xcor] of nearby-wolves
      let avg-y mean [ycor] of nearby-wolves
      if (avg-x != xcor) or (avg-y != ycor) [
        set heading towardsxy avg-x avg-y
      ]
    ]
    [ rt random 50 lt random 50 ]  ; if no nearby wolves, move randomly
end
...
to separate  ; wolf procedure
  let nearby-wolves wolves in-radius 3  ; adjust the radius as needed
  ifelse any? nearby-wolves
    [ let avg-heading mean [heading] of nearby-wolves
      set heading avg-heading + 180 ]  ; turn away from the average heading
    [ rt random 50 lt random 50 ]  ; if no nearby wolves, move randomly
end
  ...
  if prey != nobody  [                          ; did we get one? if so,
    ...
    if count sheep = 10 [
      ask patches with [pcolor = green] [
        set pcolor yellow ; change the color to a darker shade
      ]
    ]
  ]
end
